...
DLC_cell = basic_configuration_v2(DLC_cell); 
...
for row_xls = 2:size(DLC_cell,1) % first row contains labels
    ...
    % create batch/shell -script
        ...
        % create shell-script for turbsim 
        create_script_v2('turbsim',turbsim_files,join([file_path,'/wind/',DLC_name]));
        % create shell-script for FAST
        create_script_v2('openfast',main_files,join([file_path,'/sim/',DLC_name]));
end
